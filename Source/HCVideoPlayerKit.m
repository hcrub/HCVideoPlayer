...
@interface HCVideoPlayerKit () <UIGestureRecognizerDelegate>

@property (readwrite, strong) NSDictionary *currentVideoInfo;

@property (readwrite, strong) HCVideoPlayerView *videoPlayerView;
...
@property (nonatomic, strong) HCFullScreenViewController *fullscreenViewController;
@property (nonatomic) CGRect previousBounds;
@property (nonatomic) BOOL hideTopViewWithControls;

@end

@implementation HCVideoPlayerKit {
    BOOL playWhenReady;
    BOOL scrubBuffering;
...
{
    HCVideoPlayerKit *videoPlayer = [[HCVideoPlayerKit alloc] initWithContainingViewController:containingViewController
...
- (void)setControlsEdgeInsets:(UIEdgeInsets)controlsEdgeInsets
{
    if (!self.videoPlayerView) {
        self.videoPlayerView = [[HCVideoPlayerView alloc] initWithFrame:CGRectZero];
    }
...
- (void)launchFullScreen
{
    if (!self.fullScreenModeToggled) {
        ...
        if (!self.fullscreenViewController) {
            self.fullscreenViewController = [[HCFullScreenViewController alloc] init];
...
//                self.videoPlayerView.frame = self.videoPlayerView.superview.bounds;
                self.videoPlayerView.frame = CGRectMake(self.videoPlayerView.superview.bounds.origin.x,
                                                        self.videoPlayerView.superview.bounds.origin.y,
                                                        self.videoPlayerView.superview.bounds.size.width,
                                                        self.videoPlayerView.superview.bounds.size.height/2);
            }
...
- (void)updatePlaybackProgress
{
    ...
    __weak HCVideoPlayerKit *vpvc = self;
